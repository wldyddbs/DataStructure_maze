...
#첫 길찾기가 완료되면 스페이스바나 엔터키를 누르시면 correct_path가 실행됩니다.
...
def DFS(stdscr):
    s = Stack()
    num = 0
    s.push((1, 0, num))  #출발점 좌표
    countnum.push((1,0,num))

    while not s.isEmpty():
        here = s.pop()
        x, y, _ = here
        countnum.valid_push(here)
        ...
        if maze[x][y] == 'x':
            ...
            print(s)
            stdscr.refresh()
            stdscr.getch() #엔터를 입력하면 correct_path()를 수행합니다.
            return True
        ...
        if isValidPos(x, y-1): s.push((x, y-1,num))
        if isValidPos(x, y+1): s.push((x, y+1,num))  
        if isValidPos(x-1, y): s.push((x-1, y,num))
        if isValidPos(x+1, y): s.push((x+1, y,num))  
